...
import type { SanityImageWithMetadata, SanityImageAsset } from './types';
...
export interface ImageOptions {
  ...
  ignoreImageParams?: boolean; // Ignore hotspot and crop
}
...
export function buildImageUrl(image: SanityImageSource, options: ImageOptions = {}): string {
  if (!image) {
...
export function buildResponsiveImageSet(
  ...
  const imageWithMeta = image as SanityImageWithMetadata;
  ...
  // Generate sizes attribute (can be customized based on layout)
...
export function getImageHotspot(image: SanityImageWithMetadata): { x: number; y: number } | null {
...
export function getImageCrop(image: SanityImageWithMetadata): {
  top: number;
  bottom: number;
  left: number;
  right: number;
...
// Get image metadata
export function getImageMetadata(image: SanityImageWithMetadata): {
  width: number;
  height: number;
  aspectRatio: number;
  lqip?: string;

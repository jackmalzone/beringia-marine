/**
 * Targeted seed for the Independent Robotics partner.
 *
 ...
 *
 * Writes ONLY the `partner.independent-robotics` document so we don't re-upload the
...
async function main() {
  ...
  const doc = await buildPartnerDocument(independentRobotics as unknown as PartnerJson);
...
main().catch((err) => {
  console.error('\nseed-independent-robotics failed:', err);
  process.exit(1);
});

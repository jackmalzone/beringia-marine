...
describe('Image SEO Optimization', () => {
  describe('generateOptimizedAltText', () => {
    it('should generate SEO-optimized alt text with service keywords', () => {
      ...
      expect(result).toContain('premium service session');
      expect(result).toContain('local studio');
    ...
    it('should add location context for local SEO', () => {
      ...
      expect(result).toContain('local studio');
      expect(result).toContain('heated studio session');
    });

    it('should handle founder context appropriately', () => {
      const result = generateOptimizedAltText('Sean, Co-Founder', 'founder');

      expect(result).toContain('Premium Service Business');
    });

    it('should not duplicate existing keywords', () => {
      const result = generateOptimizedAltText(
        'Cold plunge therapy session at Premium Service Business Riverton',
        'service',
        'cold-plunge'
      );

      const rivertonOccurrences = (result.match(/Riverton/g) || []).length;
      expect(rivertonOccurrences).toBe(1);
    });
  });
  ...
  describe('optimizeImageProps', () => {
    it('should optimize image props with SEO enhancements', () => {
      const imageData: ImageSEOData = {
        src: 'https://example.org/coldplunge_woman.jpg',
      ...
      expect(result.alt).toContain('premium service session');
      expect(result.alt).toContain('Premium Service Business');
    ...
    it('should set priority for logo images', () => {
      const imageData: ImageSEOData = {
        src: 'https://example.org/placeholder-media',
  ...
  describe('validateImageAccessibility', () => {
    it('should validate compliant images', () => {
      const imageProps = {
        src: 'test.jpg',
        alt: 'Professional service session for clients at Premium Service Business',
    ...
    it('should recommend SEO improvements', () => {
      ...
      expect(result.recommendations).toContain(
        'Consider adding relevant service or studio keywords to alt text'
      );
  ...
  describe('batchOptimizeImages', () => {
    it('should optimize multiple images', () => {
      const images: ImageSEOData[] = [
        ...
        {
          src: 'https://example.org/logo-dark.png',
          alt: 'Logo',
          context: 'logo',
        },
      ];
      ...
      expect(results[0].alt).toContain('premium service session');
      expect(results[1].alt).toContain('Premium Service Business');
    });
  });
  ...
  describe('generateImageAuditReport', () => {
    it('should generate comprehensive audit report', () => {
      const images: ImageSEOData[] = [
        {
          src: 'test1.jpg',
          alt: 'Professional studio session at Premium Service Business',
          context: 'service',
        },
        {
          src: 'test2.jpg',
...
describe('Service-specific optimizations', () => {
  ...
  it('should detect red light therapy images', () => {
    ...
    expect(result).toContain('protocol-led session');
    expect(result).toContain('Premium Service Business');
  });
});

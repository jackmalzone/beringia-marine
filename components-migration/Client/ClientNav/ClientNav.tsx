...
const ClientNav = ({ clientSlug, sectionRefs }: ClientNavProps) => {
  ...
  const [isAnchorBot, setIsAnchorBot] = useState(false)
  ...
  // Fetch client data to check for 3D model and client type
  useEffect(() => {
    const checkClientFeatures = async () => {
      try {
        const data = await fetchClientBySlug(clientSlug)
        if (data) {
          setHas3DModel(data.modelId === SKETCHFAB_MODEL_IDS.HYDRUS_SHIPWRECK)
          setIsAnchorBot(data.slug.current === 'anchor-bot')
        }
      } catch (err) {
        console.error('Error checking client features:', err)
      }
    }
